...
router.get('/me', requireAuth, async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, name, email, phone, about, city, state, country, languages, gender, avatar_url AS avatarUrl
       FROM users WHERE id = ?`,
      [uid]
    );
    if (!rows.length) return res.status(404).json({ error: 'User not found' });
    const u = rows[0];
    // Ensure languages is array
...
router.put('/me', requireAuth, async (req, res, next) => {
    try {
      ...
      // keep languages as JSON text in DB

...
echo ">>> 1. Creating User (gopi@menon.com)..."
TRAVELER_EMAIL="gopi@menon.com"
TRAVELER_PASS="password123"
curl -s -X POST "$TRAVELER_API/auth/signup" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Gopi Menon\",\"email\":\"$TRAVELER_EMAIL\",\"password\":\"$TRAVELER_PASS\"}" > /dev/null
echo "User created: $TRAVELER_EMAIL"
...
echo "User ID: $TRAVELER_ID"

echo ">>> 3. Becoming Host..."
# Use cookies to enable host
...
echo ">>> 5. Creating Booking (Self-Booking)..."
# Create booking as the same user
...
echo ">>> 7. Accepting Booking..."
ACCEPT_RES=$(curl -s -b traveler_cookies.txt -X POST "$OWNER_API/bookings/$BOOKING_ID/accept")
